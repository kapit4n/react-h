...
export default function RoomView(props) {
  const [roomInfo, setRoomInfo] = useState(
    RoomsService.getRoomById(props.match.params.id)
  );

  return (
    <Card className="card-view" style={{ width: "100%" }}>
      ...
      <Card.Body>
        <ButtonToolbar>
          <Link className="btn btn-primary" to={`/rooms-edit/${roomInfo.id}`}>
            Edit
          </Link>{" "}
          <br />
          <Button
            to={`/rooms-edit/${roomInfo.id}`}
            onClick={() => console.log("Make the reservation")}
